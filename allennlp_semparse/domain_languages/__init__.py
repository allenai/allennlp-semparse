from allennlp_semparse.domain_languages.domain_language import (DomainLanguage, START_SYMBOL,
                                                                predicate, predicate_with_side_args)
from allennlp_semparse.domain_languages.nlvr_language import NlvrLanguage
from allennlp_semparse.domain_languages.quarel_language import QuaRelLanguage
from allennlp_semparse.domain_languages.wikitables_language import WikiTablesLanguage
