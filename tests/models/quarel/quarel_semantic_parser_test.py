...
from allennlp_semparse.nltk_languages.contexts.quarel_utils import group_worlds, to_qr_spec_string
...
class QuarelSemanticParserTest(ModelTestCase):
    ...
    def setUp(self):
        ...
        self.set_up_model(str(self.FIXTURES_ROOT / "quarel" / "experiment.json"),
    ...
    def test_elmo_model_can_train_save_and_load(self):
        param_file = self.FIXTURES_ROOT / 'quarel' / 'experiment_parser_elmo.json'
    ...
    def test_zeroshot_model_can_train_save_and_load(self):
        param_file = self.FIXTURES_ROOT / 'quarel' / 'zeroshot' / 'experiment.json'
        self.ensure_model_can_train_save_and_load(param_file, gradients_to_ignore=self.ignore)

    def test_wdp_zeroshot_model_can_train_save_and_load(self):
        param_file = self.FIXTURES_ROOT / 'quarel' / 'experiment_parser_wdp_zeroshot.json'
        self.ensure_model_can_train_save_and_load(param_file, gradients_to_ignore=self.ignore)

    def test_with_tagger_model_can_train_save_and_load(self):
        param_file = self.FIXTURES_ROOT / 'quarel' / 'experiment_parser_w_tagger.json'
        self.ensure_model_can_train_save_and_load(param_file, gradients_to_ignore=self.ignore)

    def test_entity_bits_model_can_train_save_and_load(self):
        param_file = self.FIXTURES_ROOT / 'quarel' / 'experiment_parser_entity_bits.json'
        self.ensure_model_can_train_save_and_load(param_file, gradients_to_ignore=self.ignore)

    def test_friction_model_can_train_save_and_load(self):
        param_file = self.FIXTURES_ROOT / 'quarel' / 'experiment_parser_friction.json'
        self.ensure_model_can_train_save_and_load(param_file, gradients_to_ignore=self.ignore)

    def test_friction_zeroshot_model_can_train_save_and_load(self):
        param_file = self.FIXTURES_ROOT / 'quarel' / 'experiment_parser_friction_zeroshot.json'  # pylint: disable=line-too-long
        self.ensure_model_can_train_save_and_load(param_file, gradients_to_ignore=self.ignore)

    def test_denotation_only_model_can_train_save_and_load(self):
        param_file = self.FIXTURES_ROOT / 'quarel' / 'experiment_denotation_only.json'
        self.ensure_model_can_train_save_and_load(param_file, gradients_to_ignore=self.ignore)

    def test_tagger_model_can_train_save_and_load(self):
        param_file = self.FIXTURES_ROOT / 'quarel' / 'tagger' / 'experiment.json'
        self.ensure_model_can_train_save_and_load(param_file, gradients_to_ignore=self.ignore)
